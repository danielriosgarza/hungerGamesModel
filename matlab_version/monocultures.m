...
xa=0.00677;  % Bh consuming threhalose (0.00677 for monoculture)
xb=0;        % Bh consuming glucose
xc=0;        % Bh inactive
xd=0;        % Bh dead
xe=0;        % Bt consuming glucose (0.0677 for monoculture)
xf=0;        % Bt consuming mannose
xg=0;        % Bt inactive
xh=0;        % Bt dead
xi=0;        % Ri consuming glucose (0.004 for monoculture)
...
if trans > 0 
    [t x] = ode45(@myequa,ttrans,v0,option);
    v0=x(end,:);
end

[t x] = ode45(@myequa,tspan,v0,option);
...
dxa=xa*(Exa*mumaxxa*(s1/(s1+Kxas1)+s2/(s2+Kxas2))-(Z1+Z3))+xb*Z2;
dxb=xb*(Exb*mumaxxb*(s3/(s3+Kxbs3)*s4/(s4+Kxbs4)+s2/(s2+Kxbs2))-(Z2+Z4))+xa*Z1;
dxc=xa*Z3+xb*Z4-xc*Z5;
dxd=xc*Z5;
dxe=xe*(Exe*mumaxxe*(s2/(s2+Kxes2)+s3/(s3+Kxes3))-(Z6+Z7))+xf*Z10;
dxf=xf*(Exf*mumaxxf*(s7/(s7+Kxfs7))-(Z8+Z10))+xe*Z6;
dxg=xe*Z7+xf*Z8-xg*Z9;
dxh=xg*Z9;
dxi=xi*(Exi*mumaxxi*(s3/(s3+Kxis3)+s2/(s2+Kxis2))-(Z11+Z12))+xj*Z15;
dxj=xj*(Exj*mumaxxj*(s5/(s5+Kxjs5)+s6/(s6+Kxjs6))-(Z15+Z13))+xi*Z11;
dxk=xj*Z13-xk*Z14;
dxl=xk*Z14;
...
dv=[dxa;dxb;dxc;dxd;dxe;dxf;dxg;dxh;dxi;dxj;dxk;dxl;ds1;ds2;ds3;ds4;ds5;ds6;ds7;ds8;ds9;ds10];
