...
def getExpData(modelGenes, expDict):
    ...
    for gene in modelGenes:
        ...
        expr.append(np.array(list(map(float, expDict[gene]))))
...
in_genExp = os.path.join(Path(os.getcwd()).parents[2], 'files', 'strainSummaries', 'ri','genes')

out_modelPath = os.path.join(in_modelPath, 'efms')

model = cobra.io.read_sbml_model(os.path.join(in_modelPath, 'ri_final.xml'))
...
with open(os.path.join(in_genExp, 'ri_tpm.txt') ) as f:
    f.readline()
    for line in f:
        a= line.strip().split('\t')
        geneNormedExp[a[1]] = a[4::]
...
t04,t12,x,y, d1, up, down, upDownNeutral, top, bot = getGroupComparison([0,1,2], [3,4,5], reactions, trExp)
